...
def _ol_identifiers_from_google_identifiers(google_identifiers):
    identifiers = {
        ...
        for identifier in google_identifiers
    }
...
def _upload_ol_book(ol_book):
    existing_book = OL.Work.search(
        title=ol_book.title, author=ol_book.primary_author.name
    )
    if existing_book is not None:
        raise ValueError(
            "It looks like this book already exists on Open Library. "
