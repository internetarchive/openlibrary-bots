...
class FixPromiseItems:
  def __init__(self, in_file, state_file, error_file, ol=None, batch_size=DEFAULT_BATCH_SIZE, start_line=DEFAULT_START_LINE, dry_run=True):
    self.ol = ol
  ...
  def update_edition(self, edition):
    ...
    local_id = next((b for b in edition.local_id if b.startswith("urn:bwbsku:")), "")

    # Fix local IDs:
    updated_ids = [local_id] + [_id for _id in edition.local_id if not _id.startswith("urn:bwbsku:")]
    ...
    source_record = next((r for r in edition.source_records if r.startswith("promise:")), "")
    edition.source_records = [f'{source_record}:{sku}'] + [r for r in edition.source_records if not r.startswith('promise:')]
...
def _parse_args():
  def config_file_subparser(subparsers):
    parser = subparsers.add_parser('config', help='Configure bot via configuration file')
    parser.add_argument(
      'config',
      metavar='config_path',
  ...
  def command_line_subparser(subparsers):
    ...
    parser.add_argument(
      'infile',
      help='Path to file containing program input'
    )
    parser.add_argument(
      '--error-file',
...
def configure_and_start(args):
  ...
  args.start_line = start_line if start_line else config_args.getint('start_line', DEFAULT_START_LINE)
...
def start_job(args):
  if not args.dry_run:
    ...
    num_processed = FixPromiseItems(args.infile,
      args.state_file,
      args.error_file,
      ol=ol,
