...
from lang import *
from onix import OnixHandler, OnixProduct, onix_codelists
...
def process_product(p):
    ...
    for c in op["Contributor":]:
        ...
        for pdate in c["PersonDate":]:
            ...
            if role == "007":
                ...
                author["death_date"] = date
            else:
                die("bad date role: %s" % role)
    ...
    for text in op["OtherText":]:
        ...
        if format not in ("00", "02", "07"):  # ASCII, HTML, Basic ASCII
            raise Exception(
                "unsupported description format: %s"
                % self.codelists["List34"][format][0]
            )
