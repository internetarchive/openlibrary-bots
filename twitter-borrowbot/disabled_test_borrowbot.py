...
class TestBorrowBot(unittest.TestCase):
    def test_goodreads_to_isbn(self):
        tweet = f"Hey @borrowbot, what about {goodreads_url}"
        isbns = list(ISBNFinder.find_isbns(tweet))
        assert isbns == ["9780393316049"]

    def test_twitterized_to_isbn(self):
        tweet = f"Hey @borrowbot, what about {twitterized_url}"
        isbns = list(ISBNFinder.find_isbns(tweet))
        assert isbns == ["1798434520"]

    def test_amazon_to_isbn(self):
        tweet = f"Hey @borrowbot, what about {amazon_product_url}"
        isbns = list(ISBNFinder.find_isbns(tweet))
        assert isbns == ["0393316041"]

        tweet = f"Hey @borrowbot, what about {amazon_dp_url}"
        isbns = list(ISBNFinder.find_isbns(tweet))
    ...
    def test_isbns(self):
        ...
        tweet = f"Hey @borrowbot, what about {isbn}"
        print(tweet)
        isbns = list(ISBNFinder.find_isbns(tweet))
        print(isbns)
        assert isbns == [isbn]

        tweet = f"Hey @borrowbot, what about {multiple_isbns}"
        print(tweet)
        isbns = list(ISBNFinder.find_isbns(tweet))
    ...
    def test_get_book(self):
        isbn = "0399143904"
        tweet = f"Hey @borrowbot, what about {isbn}"
        isbn = next(iter(ISBNFinder.find_isbns(tweet)))
