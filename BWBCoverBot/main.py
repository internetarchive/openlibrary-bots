...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("bwb-cover-bot-debug.log"),
...
def parser_for_zip_with_isbns(cover_zip_path: str) -> None:
    logging.info(f"start time: {datetime.datetime.now().timestamp()} for {cover_zip_path}")
    ...
    for file in file_list:
        ...
        try:
            ...
            logging.error(f'file: {file.filename}, zip path: {cover_zip_path}. Error: {e}')

    logging.info(f"end time: {datetime.datetime.now().timestamp()} for {cover_zip_path}")
